...
| OJ submission ID, if submitted | 602398 |
| OJ status | Pass |
...
```text
วิธีการทำงานของสามเหลี่ยมที่โจทย์กำหนด
```
...
```text
เลขคู่ และเลขคี่ในแต่ละแถวมีผลต่อคำตอบ
```
...
```text
ฉันเอาแนวคิดที่AIให้มาปรับใช้กับcode แล้วทดสอบoutput ว่าตรงหรือไม่
```
...
```text
ในแผนแรกผมไม่ได้ถามAI ทำให้ไม่ทราบว่าปราสาทไม่ได้มีแค่ในรูป จนได้ถามAI จึงรู้ว่าโจทย์ไม่ได้มีแค่นี้
